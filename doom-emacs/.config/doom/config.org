...
** DevContainer TRAMP Configuration
...
(after! tramp
  ...
  ;; Helper function to open files in devcontainer
  (defun open-in-devcontainer (file)
    "Open FILE in the devcontainer for the current project."
    (interactive "FFile: ")
    (let* ((project-root (doom-project-root))
           (relative-file (file-relative-name (expand-file-name file) project-root))
           (container-id (ensure-devcontainer-running project-root)))
      (if container-id
          (find-file (format "/devcontainer:%s:%s" 
                             container-id
                             (expand-file-name file)))
        (message "Failed to start or identify the devcontainer"))))

  ;; Helper function to run a shell in the devcontainer
  (defun shell-in-devcontainer ()
    "Open a shell in the devcontainer for the current project."
    (interactive)
    (let* ((project-root (doom-project-root))
           (container-id (ensure-devcontainer-running project-root)))
      (if container-id
          (let ((default-directory (format "/devcontainer:%s:/" container-id)))
            (shell))
        (message "Failed to start or identify the devcontainer"))))
)
#+end_src
